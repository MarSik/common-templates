...
%-start-and-stop: %.pvc
	oc process --local -f "templates/$*.yaml" NAME=$@ PVCNAME=$* | \
	  kubectl apply -f -
	virtctl start $@
	while ! kubectl get vmi $@ -o yaml | grep "phase: Running" ; do kubectl describe vm $@ ; kubectl describe vmi $@ ; kubectl describe pods ; kubectl -n kube-system logs -l kubevirt.io=virt-handler --tail=20 ; sleep 6; done
...
%.pv: %.raw
	...
	mkdir -p "pvs/$*" && \
	ln $< pvs/$*/disk.img && \
	sudo chown 107:107 pvs/$*/disk.img && \
